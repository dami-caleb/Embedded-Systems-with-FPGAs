	...
	if (dyn_obst.coord_x_front < V_WIDTH)
		{
			...
			if (dyn_obst.coord_x_back < V_WIDTH)
				 x_range = dyn_obst.coord_x_back-1;
...
void generate_obstacle(uint16_t *obstacles_count, uint8_t *active_obstacles, struct dynamic_obstacle obstacles_list[DYN_OBST_BUFF_SIZE])
{
    ...
    if (*active_obstacles == 0 || (V_WIDTH - obstacles_list[0].coord_x_back) >= NEW_OBSTACLE_DIST)
        {
            /*printf("GENERATING NEW OBSTACLE\n");
            int ao_buff = *active_obstacles;
            printf("Active obstacles: %i\n", ao_buff);*/
            ...
            *obstacles_count += 1;
            ...
            /*printf("NEW OBSTACLE GENERATED!\n");
            int id_buff = obstacles_list[0].id;
            printf("ID0: %i\n", id_buff);
            int top_y_buff = obstacles_list[0].coord_y_top;
            printf("OBST TOP Y0: %i\n", top_y_buff);
            int ao_buff = active_obstacles;

            int id1_buff = obstacles_list[1].id;
            printf("ID1: %i\n", id1_buff);
            int top_y1_buff = obstacles_list[1].coord_y_top;
            printf("OBST TOP Y1: %i\n", top_y1_buff);
            ao_buff = *active_obstacles;

            printf("New active obstacles: %i\n", ao_buff);
            int oc_buff = *obstacles_count;
            printf("New obstacles count: %i\n", oc_buff);*/
